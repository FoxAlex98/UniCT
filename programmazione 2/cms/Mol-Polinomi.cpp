    ...
    Polinomio* inserimento (int c,int e)
    {
     ...
     void stampa(ostream& fout)
     {
        ...
        Polinomio* operator*(const Polinomio p1)
        {
            ...
            while(temp!=NULL)
                {
                    polinomio1[temp->esp]=temp->coeff;
                    temp=temp->succ;
                }
             while(temp1!=NULL)
                {
                    polinomio2[temp1->esp]=temp1->coeff;
                    temp1=temp1->succ;
                }
                for(int i=19;i>=0;i--)
                    {
                        for(int j=19;j>=0;j--)
                        {
                          if(polinomio1[i]!=0 && polinomio2[j]!=0 )
                            {
                                if(risultato[i+j]==0)
                                risultato[i+j]=polinomio1[i]*polinomio2[j];
                                else
                                risultato[i+j]=risultato[i+j]+polinomio1[i]*polinomio2[j];
                            }
                        }
                    }
                for(int i=19;i>=0;i--)
